 ...
 * 继承WebApplicationObjectSupport用于提供上下文ApplicationContext和ServletContext的功能
...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 * MappedInterceptor为Spring MVC拦截器接口`HandlerInterceptor`的实现类  并且是个final类 Spring3.0后出来的
	...
	/**
	 * 适配给定的拦截器对象  就是支持源生的HandlerInterceptor以及WebRequestInterceptor两种情况而已
	...
	/**
	 * 查找给定请求的处理程序，如果没有找到特定的处理程序，则回退到默认处理程序
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		//getHandlerInternal(request); 为模板方法，具体由子类来实现
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		//因为handler本身也许就是个Chain，所以此处需要判断一下
		//请求执行链 包含一个handler ，拦截器数组
		HandlerExecutionChain chain = (handler instanceof HandlerExecutionChain ?
				(HandlerExecutionChain) handler : new HandlerExecutionChain(handler));

		//我的请求地址为：`http://localhost:8080/demo_war_war/api/v1/hello`  那么lookupPath=/api/v1/hello
		String lookupPath = this.urlPathHelper.getLookupPathForRequest(request, LOOKUP_PATH);
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			if (interceptor instanceof MappedInterceptor) {
				MappedInterceptor mappedInterceptor = (MappedInterceptor) interceptor;
				//这里其实就能体现出MappedInterceptor的些许优势了：也就是它只有路径匹配上了才会拦截，
				// 没有匹配上的就不会拦截了，处理起来确实是更加的优雅些了
